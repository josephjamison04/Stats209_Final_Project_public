...
ggplot(data = data, aes(x = ket_per_kg)) +
  geom_histogram() +
  facet_wrap(~ z)
```
...
ate
[1]  0.71875000  0.09523810  0.22807018  0.09756098 -0.25000000
...
for (i in seq_along(min_ket_dose)) {
  ...
  print(mean(predict(lm(day7 ~ age + bmi + sex + a_los + day0, data = data_subset, subset = z == 1), newdata = data_subset) - predict(lm(day7 ~ age + bmi + sex + a_los + day0, data = data_subset, subset = z == 0), newdata = data_subset)))
