...
```{r}
data_subset <- obs_data %>% filter((ket_per_kg >= 0.5) | (z == 0))
data_subset$prop <- glm( z ~ age + sex + bmi + a_los + day0,                     family = binomial,                         data = data_subset)$fitted.values
data_subset$z <- data_subset$surg_type__16
compute_aipw_estimator(data_subset)
```